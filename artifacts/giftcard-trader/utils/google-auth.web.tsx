import React from "react";
import { TouchableOpacity, View, Text, StyleSheet, ActivityIndicator } from "react-native";
import Svg, { Path } from "react-native-svg";

export const statusCodes = {
  SIGN_IN_CANCELLED: "SIGN_IN_CANCELLED",
  IN_PROGRESS: "IN_PROGRESS",
  PLAY_SERVICES_NOT_AVAILABLE: "PLAY_SERVICES_NOT_AVAILABLE",
  SIGN_IN_REQUIRED: "SIGN_IN_REQUIRED",
};

type GoogleSignInResult =
  | { type: "cancelled" }
  | { type: "success"; data: { idToken: string | null; user: { id: string; name: string | null; email: string; photo: string | null; familyName: string | null; givenName: string | null } } };

export const GoogleSignin = {
  configure: (_options?: Record<string, unknown>) => {},
  signIn: async (): Promise<GoogleSignInResult> => ({ type: "cancelled" }),
  signOut: async () => {},
  isSignedIn: () => false,
  hasPlayServices: async () => true,
};

function GoogleG({ size = 18 }: { size?: number }) {
  return (
    <Svg width={size} height={size} viewBox="0 0 48 48">
      <Path fill="#EA4335" d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z" />
      <Path fill="#4285F4" d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.65z" />
      <Path fill="#FBBC05" d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24c0 3.88.92 7.54 2.56 10.78l7.97-6.19z" />
      <Path fill="#34A853" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.15 1.45-4.92 2.3-8.16 2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z" />
    </Svg>
  );
}

interface GoogleSigninButtonProps {
  onPress: () => void;
  disabled?: boolean;
  style?: any;
  size?: number | string;
  color?: string;
  loading?: boolean;
}

export function GoogleSigninButton({ onPress, disabled, style, loading }: GoogleSigninButtonProps) {
  return (
    <TouchableOpacity
      onPress={onPress}
      disabled={disabled || loading}
      activeOpacity={0.82}
      style={[gStyles.btn, disabled && gStyles.btnDisabled, style]}
    >
      <View style={gStyles.logoWrap}>
        <GoogleG size={18} />
      </View>
      <View style={gStyles.separator} />
      {loading ? (
        <ActivityIndicator size="small" color="#1F1F1F" style={gStyles.spinner} />
      ) : (
        <Text style={gStyles.label}>Sign in with Google</Text>
      )}
    </TouchableOpacity>
  );
}

const gStyles = StyleSheet.create({
  btn: {
    flexDirection: "row",
    alignItems: "center",
    backgroundColor: "#FFFFFF",
    borderWidth: 1,
    borderColor: "#747775",
    borderRadius: 4,
    height: 44,
    overflow: "hidden",
  },
  btnDisabled: { opacity: 0.6 },
  logoWrap: {
    width: 44,
    alignItems: "center",
    justifyContent: "center",
  },
  separator: {
    width: 1,
    alignSelf: "stretch",
    backgroundColor: "#747775",
    opacity: 0.35,
  },
  label: {
    flex: 1,
    textAlign: "center",
    fontSize: 14,
    fontFamily: "Inter_500Medium",
    color: "#1F1F1F",
    letterSpacing: 0.25,
  },
  spinner: { flex: 1 },
});
