...
- **Tamagui** — `tamagui` + `@tamagui/config` installed, config at `tamagui.config.ts`. **TamaguiProvider NOT in tree** — its wrapper div breaks flex layout on Expo web. Instead, custom `XStack`/`YStack`/`ZStack` components at `components/Stacks.tsx` provide the same API using React Native View with numeric prop values (gap, padding, borderRadius, etc.).
- **Stacks** `components/Stacks.tsx` — Lightweight `XStack` (row), `YStack` (column), `ZStack` (layered) components. Props: `gap`, `padding`, `paddingHorizontal`, `paddingVertical`, `alignItems`, `justifyContent`, `flex`, `backgroundColor`, `borderRadius`, `borderWidth`, `borderColor`, `width`, `height`, `style`. Used in Home (header, quick actions, section headers), Wallet (header, stats, KYC banner, action buttons, assets, transactions, modal).
- **Typography** `components/Typography.tsx` — `SizableText` (sized text with `size` prop: `$1`–`$10`), `Paragraph` (semantic text), `Strong` (bold inline), `Em` (italic inline), `Span` (plain inline). Size tokens map to Inter font sizes (11–36px) with matching line heights. Font weights auto-resolve to Inter family variants (400=Regular, 500=Medium, 600=SemiBold, 700=Bold). Used in Home (greeting, username, section titles, FAB label), Wallet (all headers, balance card, KYC banner, action labels, asset names/values, transaction items, modal content). Replaces raw `Text` imports in these screens.
